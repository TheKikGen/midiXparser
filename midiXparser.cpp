...
// DESTRUCTOR IS MANDATORY TO FREE SYSEX BUFFER
midiXparser::~midiXparser() {
...
// Return the type of a midi status
...
// Return the len of the last parsed midi message 
    ...
    // Midi Data from 00 to 0X7F --------------------------------------------------
          ...
          if ( m_nextMidiMsglen ) {
            ...
            if (m_nextMidiMsglen == 0) {
                m_midiParsedMsgType = m_midiCurrentMsgType;
                if (m_midiParsedMsgType == channelVoiceMsgType) m_runningStatusPossible = true;
                return true;
            }
